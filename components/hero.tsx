...
export function Hero() {
  return (
    <section id="home" className="scroll-mt-[80px] flex flex-col items-center justify-center text-center px-6 pt-[100px] pb-12 md:pb-20 md:min-h-screen relative overflow-x-hidden bg-white">
      <div className="absolute w-[600px] h-[600px] rounded-full bg-[radial-gradient(ellipse,rgba(26,92,42,0.08)_0%,transparent_70%)] -top-[100px] -right-[100px] pointer-events-none" />
      <div className="absolute w-[500px] h-[500px] rounded-full bg-[radial-gradient(ellipse,rgba(74,158,99,0.12)_0%,transparent_70%)] -bottom-20 -left-20 pointer-events-none" />

      <div
        className="absolute inset-0 opacity-50 pointer-events-none"
        style={{
          backgroundImage: `linear-gradient(rgba(26,92,42,0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(26,92,42,0.1) 1px, transparent 1px)`,
      ...
      <p className="animate-fade-up text-sm text-text2 mb-6">
        Launching first in <span className="font-semibold text-bark">{BRAND.location}</span>
        {" · "}
        Expanding across the USA
      </p>

      <h1 className="animate-fade-up-delay-1 font-serif text-[clamp(44px,6.5vw,86px)] font-medium leading-[1.06] tracking-[-0.03em] max-w-[860px] mb-7 text-bark text-balance">
      ...
      <div className="animate-fade-up-delay-3 w-full max-w-[620px]">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          ...
          <SectionLink
            href="#features"
            className="inline-flex items-center justify-center w-full sm:w-auto sm:min-w-[180px] font-sans text-sm font-semibold text-bark border border-border-strong bg-white/70 hover:bg-white px-8 py-3.5 rounded-full transition-all hover:-translate-y-0.5 hover:shadow-lg"
          >
            Explore Features
          </SectionLink>
        </div>
      ...
      <div className="animate-scale-in-delay-4 relative w-full max-w-[900px] mt-8 md:mt-10 px-0">
        <div
          className="absolute left-1/2 top-[55%] -z-10 h-[70%] w-[85%] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[radial-gradient(ellipse,rgba(26,92,42,0.42)_0%,rgba(74,158,99,0.22)_45%,transparent_72%)] blur-3xl"
          aria-hidden
        />
        <Image
          ...
          className="relative w-full h-auto drop-shadow-[0_28px_70px_rgba(26,92,42,0.38)]"
          priority
        />
      </div>
      ...
      <div className="animate-fade-up-delay-5 sm:hidden flex gap-3 mt-10 w-[calc(100%+3rem)] -mx-6 px-6 overflow-x-auto snap-x snap-mandatory pb-3 scrollbar-none">
...
function HeroImage({
  ...
  return (
    <div className={`rounded-2xl overflow-hidden relative shadow-xl ${className}`}>
      ...
      />
      <span className="absolute bottom-3 left-3 bg-bark/85 backdrop-blur-lg text-white text-[11px] font-semibold tracking-[0.06em] uppercase px-3 py-1.5 rounded-full">
        {label}
      </span>
    </div>
  )
